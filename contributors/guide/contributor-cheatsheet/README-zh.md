...
这是一份参与 Smarter 协作时的常用资源清单，包含 Smarter 项目中常用的提示、技巧和最佳实践。这个「长话短说」或快速参考包含一些有用的信息，能让您有更好的 GitHub 贡献体验。
...
- [贡献者指南] - 如何开始参与 Smarter 协作的指南
- [开发者指南] - 直接向 Smarter 项目贡献代码的指南
...
- [日历] - 查看所有的 Smarter 社区事件（SIG 或 WG 会议、活动等）
- [kubernetes-dev] - Smarter 开发邮件列表
- [Smarter 论坛] - Smarter 官方论坛
- [Slack 频道] - Smarter 官方 Slack
- [Stack Overflow] - Smarter 用户提问的地方
- [YouTube 频道] - Smarter 社区的官方频道


### 工作流程

- [Prow] - Smarter CI/CD 系统
- [Tide] - 管理合并和测试的 Prow 插件 [Tide 仪表盘]
- [Bot 命令] - 用来和 Smarter 机器人互动的命令（例如 `/cc`、`/lgtm` 和 `/retest`）
- [GitHub 标签] - Smarter 整个项目中使用的标签列表
- [Smarter 代码搜索], 由 [@dims] 维护
...
在您提交贡献之前，您必须[签署贡献者许可证协议（CLA）][cla]。_只有_ 您或者您的公司签署了 CLA，Smarter 项目才能接受您的贡献。
...
GitHub Issue 是追踪 bug 报告、改善请求，或者报告例如失败测试的其它问题的主要途径。它们**不**应该用来发布[用户支持请求]。对于那些问题，请查看[故障排除指南]，在[Stack Overflow] 报告，或者在 [Smarter 论坛] 跟进。
...
- 尽量详细描述您提交的 issue。
- 使用合适的[标签]。如果您不确定，[k8s-ci-robot][prow] 机器人（[Smarter CI 机器人][prow]）将会回复您的 issue 并为其添加需要的标签，让分派更高效。
...
在您的 PR 提交后，一系列测试将由 Smarter CI 平台 [Prow] 运行。如果任何测试失败，[k8s-ci-robot][prow] 将会回复该 PR，附上失败测试及其 log 的链接。

向您的 PR push 新的提交将会自动触发测试重新运行。

Smarter CI 平台偶尔会出现问题。即便您的贡献通过了所有本地测试，也会因为其它各种原因发生问题。您可以使用 `/retest` 命令触发重新运行测试。
...
Smarter 使用[标签]来分类和分派 issue 和 Pull Request。使用正确的标签将会帮助您的 issue 或 PR 得到更高效的分派。
...
Smarter 项目使用 GitHub 标准的 _「Fork 然后 Pull」_ 的工作流程。在 git 术语中，您个人的 fork 被称为 _「`origin`」_，实际的项目 git 仓库被称为 _「`upstream`」_。为了保证您的个人分支（`origin`）与项目（`upstream`）保持更新，必须要在您的本地工作副本中进行设置。
...
[Smarter 论坛]: https://discuss.kubernetes.io/
